...
firebase_url = 'https://building-automation-80d7d-default-rtdb.firebaseio.com/'
# Step 1: Load Firebase credentials
cred = credentials.Certificate("C:/Users/LENOVO/Downloads/building-automation-80d7d-firebase-adminsdk-iy7t4-00ef05d62d.json")  # Replace with your actual file path
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://building-automation-80d7d-default-rtdb.firebaseio.com/'
})
############################################# FUNCTIONS ################################################
...
def registration():
    ...
    save = tk.Button(frame2, text="SAVE PROFILE", command=saveprofile, fg="black", bg="#3ece48", width=15,activebackground="white", font=('times', 15, ' bold '))
...
def make_icard():
    ...
    draw.text((60, 645), text='USER ADDRESS', fill='white', font=font)
    draw.text((20, 20), text='FLAT NAME & LOGO', fill='white', font=font)
        ...
        if exists:
            with open("Attendance\Attendance_" + date + ".csv", 'r') as csvfile3:
                ...
                for lines in reader1:
                    ...
                    if (j > 1) & (j % 2 != 0):
                        if (lines[0] == rfid):
                            mess._show(title='User Entry Marked',message='Have a Nice Day')
                            k=0
                            break
                        else:
                            pass
            ...
            with open("Details\Details.csv", 'r') as csvFile2:
                ...
                for lines in reader1:
                    if (str(lines[0]) == rfid):
                        ...
                        print(attendance)
                        result = requests.post(firebase_url + '/' + "DKTE" + '/entryout.json', data=json.dumps(attendance))
                        print("record sent to cloud successfully")
...
window.title("IN ENTRY Registor")
window.configure(background='#262523')

frame1 = tk.Frame(window, bg="#00aeff")
frame1.place(relx=0.12, rely=0.17, relwidth=0.76, relheight=0.80)

message3 = tk.Label(window, text="RFID Based OUT Entry System" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
message3.place(x=10, y=10)
...
head1 = tk.Label(frame1, text="**** OUT ENTRY SCREEN ****", fg="white",bg="blue" ,height=1,width=69,font=('times', 18, ' bold ') )
head1.place(x=0,y=0)
...
lbl3 = tk.Label(frame1, text="--------------- OUT ENTRY ATTENDANCE ----------------",width=50  ,fg="black"  ,bg="#00aeff" ,font=('times', 15, ' bold '))
